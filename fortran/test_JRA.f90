...
program test
 implicit none
 integer, parameter :: xdim = 288, ydim = 145
 ...
 open(11, file='relv_700hPa.grd', form='unformatted', &
 & access='direct', recl=xdim*ydim*4)
    ...
    do m=3509,3628
      read(11,rec=m) precip
        ...
        do j=1, ydim
          do i=1, xdim
            Top5ave(i,j)= (sum2006(i,j) + sum2012(i,j) + sum2009(i,j) +sum2003(i,j) +sum2011(i,j))/1076
          end do
        end do
        ...
        do j=1, ydim
          do i=1, xdim
            Bottom5ave(i,j)= (sum2004(i,j) + sum2002(i,j) + sum2017(i,j) +sum2005(i,j) +sum2013(i,j))/764
          end do
        end do
        ...
        do j=1, ydim
          do i=1, xdim
            ...
            otherAve(i,j)=(otherAve1(i,j) +otherAve2(i,j))/1788
          end do
        end do
    ...
    do m=3509,3628
      read(11,rec=m) precip
        ...
        do j=1, ydim
          do i=1, xdim
            Top5si(i,j)= (si2006(i,j) + si2012(i,j) + si2009(i,j) + si2003(i,j) + si2011(i,j))/1076
          end do
        end do

        do j=1, ydim
          do i=1, xdim
            Bottom5si(i,j)= (si2004(i,j) + si2002(i,j) + si2017(i,j) + si2005(i,j) + si2013(i,j))/764
          end do
        end do
        ...
        do j=1, ydim
          do i=1, xdim
            ...
            otherSi(i,j)=(otherSi1(i,j) +otherSi2(i,j))/1788
          end do
        end do
        ...
        do j=1, ydim
          do i=1, xdim
            tt(i,j) = abs(Bottom5ave(i,j) - otherAve(i,j)) / sqrt(Bottom5si(i,j)/764 + otherSi(i,j)/1788)
          end do
        end do
        ...
        do j=1, ydim
          do i=1, xdim
            !m1(i,j) = (Top5si(i,j)/764 + otherSi(i,j)/1788)**2
            !m2(i,j) = (Top5si(i,j)**2)/((764**2)*(764-1))
            !m3(i,j) = (otherSi(i,j)**2)/((1788**2)*(1788-1))
            !mm(i,j) = m1(i,j) / (m2(i,j) + m3(i,j))
            !mm(i,j) = nint(mm(i,j))
            mm(i,j) = 241
          end do
        end do
...
write(*,*) sig
  ...
  open(20, file='relv_ttest_Bottom5years.bin', form='unformatted', &
  & access='direct', recl=xdim*ydim*4)
  write(20,rec=1) sig
